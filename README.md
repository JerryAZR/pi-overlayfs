...
- Once per turn — at `turn_end`, after **all** tool calls in the batch have completed (pi awaits every execution before emitting it, and awaits the finisher before the next LLM call) — a finisher reviews the staged changes (real host paths) and applies them to disk:
  ...
  - **Denied changes are never silent** — the model gets a steering message before its next LLM call listing exactly which paths were discarded, so it doesn't believe its writes persisted. Tool results themselves are left untouched (a write that succeeded in the overlay is reported honestly as success).
...
- **Failed applies are retried implicitly.** If applying staged changes fails (disk full, permissions, ...), the unapplied entries stay pending in the overlay: they remain visible to subsequent reads/bash and the finisher retries them on the next `turn_end`. A notification is shown on failure.
